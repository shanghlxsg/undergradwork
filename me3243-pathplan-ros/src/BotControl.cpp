...
BotControl::BotControl(ros::NodeHandle& nh)
{
  ...
  error_pos_pub_ = nh_.advertise<std_msgs::Float32>("/error_pos", 1);           // edited by A0167431H
  error_heading_pub_ = nh_.advertise<std_msgs::Float32>("/error_heading", 1);   // edited by A0167431H
  
  // Enter the PID control parameters here:
  // Kp_a = 0.04;
  // Ki_a = 0.2;
  // Kd_a = 0;
  // Kp_x = 1;
...
// edited by A0167431H
  linear_error.data = error_pos_;
  error_pos_pub_.publish(linear_error);
// edited by A0167431H
  angle_error.data = error_heading_;
  error_heading_pub_.publish(angle_error);
